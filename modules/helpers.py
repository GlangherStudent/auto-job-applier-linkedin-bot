'''
Common helper functions for logging, timing and utilities.
...
def make_directories(paths: list[str]) -> None:
    '''
    Function to create missing directories using pathlib (cross-platform)
    '''
    from pathlib import Path

    for path_str in paths:
        try:
            path = Path(path_str).expanduser()  # Expands ~ to user's home directory

            # If path has file extension, get parent directory
            if path.suffix:
                path = path.parent

            if not path or str(path) == '.':  # Handle empty or current dir
                continue
            ...
            print(f'Error while creating directory "{path_str}": {e}')
...
def find_default_profile_directory() -> str | None:
    ...
    if sys.platform.startswith('win'):
        paths = [
            ...
            home / "Local Settings" / "Application Data" / "Google" / "Chrome" / "User Data"
        ]
    # Linux
    elif sys.platform.startswith('linux'):
        paths = [
            home / ".config" / "google-chrome",
            home / ".var" / "app" / "com.google.Chrome" / "data" / ".config" / "google-chrome",
        ]
...
def get_log_path():
    ...
    '''
    try:
        path = f"{logs_folder_path}/log.txt"
        return path.replace("//","/")
    except Exception as e:
...
def manual_login_retry(is_logged_in: callable, limit: int = 2) -> None:
    ...
    ##> ------ Enhanced by AI Audit - Auto-flow without alerts ------
    Modified to wait and retry without alerts.
    ##<
    '''
    count = 0
    max_retries = 10
    while not is_logged_in() and count < max_retries:
        print_lg("⚠️ Checking login status...")
        if count == 0:
            print_lg("⏳ Waiting for login (will retry automatically)...")
        sleep(5)  # Wait 5 seconds between checks
        count += 1
        if count >= limit:
            print_lg("✅ Assuming login successful after retries")
            return
...
def convert_to_lakhs(value: str) -> str:
    ...
    * "5000" -> "0.05" 
    ...
    if l > 0:
        if l > 5:
            value = value[:l-5] + f".{value}"[l-5:l-3]
        else:
